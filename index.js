...
function ImageServerSource(uri, callback){
    ...
    verifySource(parsed_uri['query']['source'], function(e){
        if (e) { 
            console.error("ImageServer source " + parsed_uri['query']['source'] + " is invalid. ("+JSON.stringify(e)+")")
        }
    });
...
ImageServerSource.prototype.getTile = function(z, x, y, callback){
    ...
    if (this.options.rasterFunction){ 
        params['renderingRule'] = "{\"rasterFunction\" : \""+this.options.rasterFunction+"\"}"
    ...
    function(error, response, body){ 
        ...
        switch (response.statusCode) {
            ...
            default:
                console.warn("ImageServer URL " + root + '/exportImage with params ' + JSON.stringify(params) + " returned HTTP status code " + response.statusCode + ", tile will be skipped");
